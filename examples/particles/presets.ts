...
const NATIVE_PRESETS = [
    ...
    { id: "orbital-current", category: "ENERGY", color: "#a37bff", name: "Orbital current", description: "往復する円弧から光が生まれ、軌道速度と半径方向の移動で螺旋を編む。速度に応じて色が変わる。", definition: { emitters: [
        { id: "main", options: { capacity: 128, rate: 24, drag: { coefficient: [{ time: 0, value: 0.1 }, { time: 1, value: 0.7 }], multiplyByVelocity: true }, scalingMode: "shape", sortMode: "none", renderer: { kind: "horizontal", minScreenSize: 0.004, maxScreenSize: 0.05 }, limitVelocity: { speed: [{ time: 0, value: 4 }, { time: 1, value: 1 }], dampen: 0.15 }, colorBySpeed: { range: [0, 3], curve: [{ time: 0, value: 0x8765ff }, { time: 1, value: 0xffffff }] }, orbitalVelocity: { y: [{ time: 0, value: 1.8 }, { time: 1, value: 0.6 }] }, radialVelocity: [{ time: 0, value: -0.1 }, { time: 1, value: -0.35 }], shape: { kind: "ring", radius: 1.7, arc: { angle: Math.PI * 1.5, mode: "pingPong", speed: 2 } }, velocity: P(0, 0.15), lifetimeMs: 4000, size: 0.08, color: 0xa37bff, blending: "additive", trails: { segments: 20, intervalMs: 30, width: 0.06, widthOverTrail: [{ time: 0, value: 1 }, { time: 1, value: 0 }] }, prewarmMs: 1000 } },
    ] } },
    { id: "comet", category: "COSMIC", color: "#5be7ff", name: "Ribbon flight", description: "移動を補間し、粒子を二本のリボンでつなぐ。シーンの光を受け、床に影を落とす。", definition: { emitters: [
        { id: "main", options: { capacity: 256, interpolateMotion: true, rateOverDistance: 35, simulationSpace: "world", velocity: P(), inheritVelocity: 0.25, inheritVelocityMode: "current", inheritVelocityOverLife: [{ time: 0, value: 1 }, { time: 1, value: 0.2 }], recording: { maxCommands: 4000 }, lifetimeMs: 1600, size: 0.08, color: 0x5be7ff, blending: "additive", drag: 1, trails: { mode: "ribbon", ribbonCount: 2, castShadow: true, receiveShadow: true, width: 0.07, persistMs: 650, widthOverTrail: [{ time: 0, value: 1 }, { time: 1, value: 0 }], colorOverTrail: [{ time: 0, value: 0xffffff }, { time: 1, value: 0x6941ff }] } } },
    ] } },
    { id: "shards", category: "NATURE", color: "#86dfff", name: "Crystal impact", description: "周囲を照らしながら落下するPBRの結晶。軸別の大きさ・回転カーブを持ち、衝突のたびに寿命が短くなる。", definition: { emitters: [
        { id: "main", options: { capacity: 128, lights: { maxLights: 3, ratio: 0.3, intensity: 1.5, range: 2 }, rate: 18, castShadow: true, receiveShadow: true, sizeAxesOverLife: { y: [{ time: 0, value: 0.5, interpolation: "bezier", outControl: 1.8 }, { time: 1, value: 0.4, inControl: 1.1 }] }, angularVelocityAxesBySpeed: { range: [0, 5], curves: { x: [{ time: 0, value: 0.2 }, { time: 1, value: 2 }] } }, sizeAxes: { x: [0.6, 1.3], y: [1, 2], z: [0.6, 1.2] }, rotation3D: { x: [0, 6.28], y: [0, 6.28], z: [0, 6.28] }, angularVelocity3D: { x: [-2, 2], y: [-3, 3] }, lighting: { ambient: 0.25, intensity: 1.6, direction: P(2, 3, 4) }, startColors: [0x86dfff, 0xb9a8ff, 0x6dffcf], position: P(0, 1.6), simulationSpace: "world", shape: { kind: "circle", radius: 0.7 }, velocity: P(0.35, -0.5), acceleration: P(0, -4), lifetimeMs: 3500, size: [0.12, 0.24], angularVelocity: [1, 4], color: 0x86dfff, renderer: { kind: "mesh", pivot: P(0, 0.15), flip: P(0.5, 0, 0.5), meshes: [{ positions: shard, weight: 3 }, { positions: shard.map((v, i) => i % 3 === 1 ? v * 0.5 : v * 1.4), weight: 1 }] }, collision: { colliders: [{ kind: "plane", normal: P(0, 1), offset: -1.5 }, { kind: "sphere", center: P(-0.5, -0.65), radius: 0.45 }, { kind: "box", min: P(0.4, -1.5, -0.4), max: P(1.1, -0.8, 0.4) }], bounce: 0.65, friction: 0.15, radius: 0.1 }, prewarmMs: 1000 } },
    ] } },
...
function createNativeEffect(preset: typeof NATIVE_PRESETS[number], textures: Record<TextureName, THREE.DataTexture>): Effect {
    ...
    if (preset.id === "shards" || preset.id === "comet") {
    ...
    if (preset.id === "comet") runtime.main = { trailTexture: textures.streak, trailMaterial: standard! };
    ...
    return {
        ...(preset.id === "shards" ? { sortDepth: () => effect.sort(camera, "distance", { scope: "global", maxParticles: 128 }) } : {}),
...
export function createEffect(id: PresetId, textures: Record<TextureName, THREE.DataTexture>): Effect {
    ...
    switch (id) {
        ...
        case "frost":
            ...
            pulse(layer({ color: 0x61d9ff, size: 4, lifetimeMs: 1500, spriteSheet: { columns: 2, rows: 2, frameOverLife: [{ time: 0, value: 0 }, { time: 0.3, value: 2.4 }, { time: 1, value: 3 }], blend: true }, sizeOverLife: [{ time: 0, value: 0.05 }, { time: 1, value: 1.3 }] }, "ripple"), 1, 1000); break;
        case "reactor":
