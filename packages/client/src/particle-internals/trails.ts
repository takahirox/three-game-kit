...
export function createTrails(options: ParticleEmitterOptions, capacity: number) {
    ...
    record(config, ["segments", "intervalMs", "width", "widthOverTrail", "colorOverTrail", "opacityOverTrail", "persistMs", "textureMode", "tileLength"], "trails");
    const segments = integer(config.segments ?? 12, 2, 64, "trail segments"), persist = number(config.persistMs ?? 0, 0, 1e6, "trail persistMs");
    ...
    function copy(from: number, to: number) { counts[to] = counts[from]!; heads[to] = heads[from]!; lastSample[to] = lastSample[from]!; history.copyWithin(to * segments * 3, from * segments * 3, (from + 1) * segments * 3); }
    ...
    return {
        ...
        birth(i: number, p: THREE.Vector3, time: number) { counts[i] = 1; heads[i] = 0; lastSample[i] = time; p.toArray(history, i * segments * 3); },
        ...
        sample(i: number, p: THREE.Vector3, time: number) {
            if (time - lastSample[i]! < interval) return;
        ...
        upload(active: number, time: number, centers: THREE.InstancedBufferAttribute) {
            ...
            for (let item = 0; item < active + retained; item++) {
