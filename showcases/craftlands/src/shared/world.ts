  ...
  findSpawn(): Vec3 {
    ...
    for (let radius = 0; radius <= 320 && best === null; radius += 8) {
      for (let dx = -radius; dx <= radius; dx += 8) for (let dz = -radius; dz <= radius; dz += 8) {
        ...
        if (column.height <= SEA_LEVEL + 1 || column.height > SEA_LEVEL + 24 || column.biome === 0 || column.biome === 1 || column.biome === 8 || column.biome === 5 || column.biome === 6 || column.biome === 7) continue;
        let flat = true;
        for (let nx = -3; nx <= 3 && flat; nx += 1) for (let nz = -3; nz <= 3; nz += 1) {
